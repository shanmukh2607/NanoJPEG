// Filename        : rowidct.v
// Description     : rdy signal problem fixing - sync write approach
...
module rowidct (
    ...
    assign #20mul4 = (a<<7) + (a<<5) + (a<<4) + (a<<2) + a;//181* a;
    ...
    always @(*)
        begin
            ...
            if(((x1 << 11)| x2 | x3 | x4 | x5 | x6 | x7 ) == 32'b0)
                begin
                    {Y0,Y1,Y2,Y3,Y4,Y5,Y6,Y7} = {8{x0<<3}}; // fast enough to be computed in less than a clk cycle
                    ardy = 1; // hence ardy is set 1
                end
            else
                begin
                    $display($time,"%d %d %d %d %d %d %d %d",Y0,Y1,Y2,Y3,Y4,Y5,Y6,Y7);
                    {Y0,Y1,Y2,Y3,Y4,Y5,Y6} = {yc0,yc1,yc2,yc3,yc4,yc5,yc6}; // long comb path
                    #10Y7 = yc7;
                    // But every output port has roughly same comb delay
                    // Hence if one Y-series port is ready after some time, the other ports are ready too in about the same clock cycle.
                    // If a Y series port is ready then it mismatches with atleast one final output corresponding port
                    ardy = (y0 == Y0) & (y1 == Y1) & (y2 == Y2) & (y3 == Y3) & (y4 == Y4) & (y5 == Y5) & (y6 == Y6) & (y7== Y7)  ;
                ...
                begin
                    ...
                    if(ardy)
                        begin
                            ...
                            //{y0,y1,y2,y3,y4,y5,y6,y7} <= {Y0,Y1,Y2,Y3,Y4,Y5,Y6,Y7};
                            rdy <= 1;
                            //$display("if: ardy val is %d and flag is %d",ardy,y0);
                        end
                    else
                        begin
                            //{y0,y1,y2,y3,y4,y5,y6,y7} <= {Y0,Y1,Y2,Y3,Y4,Y5,Y6,Y7};
                            rdy <= 0;
                           // $display("else: ardy val is %d and flag is %d",ardy,y0);
                        end
                end
        end
